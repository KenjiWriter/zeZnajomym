<div>
    @if ($finished == true)
        <div class="message-container">
            <p class="message">Wspaniale! Twój formularz został stworzony! Teraz wystarczy wysłać ten link do znajomego:
            </p>
            <p class="link">zeZnajomym.pl/id/{{ $newProfileId }}</p>
            <p class="description">Jak skończy wypełniać potrzebne dane będziesz mógł zobaczyć informacje dotyczące
                przejazdu w swoim profilu, do którego zalogujesz się za pomocą tych danych:</p>
            <p class="credentials">Login: {{ $newProfileId }} | Kod: {{ $code }}</p>
        </div>
    @else
        <div class="">
            ...
            <form class="form-container" wire:submit.prevent="submit">
                ...
                @if ($price_type == 1)
                    <div class="">
                        ...
                        <input class="form-input" type="text" wire:model="price_per_km"
